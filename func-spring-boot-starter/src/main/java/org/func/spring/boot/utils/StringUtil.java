 ...
 * @author Yiur
 */
public final class StringUtil implements FuncStringConstantPool {
    ...
    private static String formatInit(String input, Object... args) throws Throwable {
        String result = input;
        try {
            ...
            throw new FuncException("method format resolve error");
        }
        return result;
    }
    ...
    public static String replace(String expression, Map<String, Object> constraint) throws Throwable {
        Assert.notNull(expression, "expression is not null!");
        List<String> matchers = matchers(STRING_REPLACE, expression);
        String result = expression;
        for (String matcher : matchers) {
            result = result.replace(matcher, ScriptEngine.eval(matcher.substring(2, matcher.length() - 1), (LinkedHashMap<String, Object>) constraint).toString());
        }
        return result;
    }
