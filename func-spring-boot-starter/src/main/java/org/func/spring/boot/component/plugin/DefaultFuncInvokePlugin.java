            ...
            if (declaredConstructor.getParameterCount() == args.length) {
                try {
                    ...
                    for (Field declaredField : result.getClass().getDeclaredFields()) {
                        ...
                        // bean
                        Object variable = funcLink.getVariable(format("?:?", beanName, declaredField.getName()));
                        // ref
                        if (variable == null) {
                            for (String ref : refs) {
                               if (variable == null) {
                                   variable  = funcLink.getVariable(format("?:?", ref, declaredField.getName()));
                               }
                            }
                        }
                        // type
                        if (variable == null) {
                            variable = funcLink.getVariable(declaredField.getType());
                        }
                        if (variable != null) {
                            declaredField.set(result, variable);
                        }
                    }
